...
class StatisticsViewModel: ObservableObject {
    ...
    init() {
        // Calculate emissions and schedule notifications when the model is initialized
        calculateAndNotifyEmissions(for: Date())
    }
    
    let calendar = Calendar.current
    
    
    func calculateAndNotifyEmissions(for date: Date) {
        ...
        let emissionLimit = totalEmissionsYesterday
        let emissionDifference = totalEmissions - totalEmissionsYesterday
        
        // Schedule the daily summary notification at 10 PM
        NotificationManager.shared.scheduleDailySummaryNotification(totalEmissions: totalEmissions, emissionLimit: emissionLimit)
